...
const Header: React.FC<HeaderProps> = ({ }) => {
  ...
  return (
    <>
      <div
        className={`fixed top-0 left-0 w-full transition-all duration-300 pl-2 pr-2 z-[100] ${isScrolled ? "bg-white  py-2  border-gray-200 shadow-[0px_2px_20px_2px_rgba(0,0,0,0.2)]   " : "py-5"}  ${isMobile ? "border-gray-200 shadow-[0px_2px_20px_2px_rgba(0,0,0,0.2)]" : ""}`}
      >
        <div className={`flex w-full items-center px-10 max-md:px-5  border-gray-200   ${isScrolled ? "justify-between" : ""}`}>
          <img
            loading="lazy"
            src="./logo.gif"
          ...
          {!isMobile && (
            <nav
              className={`flex items-center gap-5 rounded-[40px] transition-all z-50 duration-300  py-2.5 px-10 text-neutral-800 mr-2 ${isScrolled ? "bg-transparent" : "bg-gray-100"}`}
              role="navigation"
            >
              ...
              <button className="bg-green-500 px-4 py-2 text-white rounded-3xl">Get Started</button>
            </nav>
          )}
        </div>
      </div>
          ...
          {["HOME", "FEATURES", "PRICING", "PERFORMANCE", "FAQ", "CONTACT"].map((label) => {
            ...
            return (
              <span
                ...
                onClick={() => {
                  setMenuOpen(false);
                  section != "contact" ? handleNavigation(section) : handleContact()
                }}
                className={`cursor-pointer ${activeLink === section ? "text-green-500" : ""}`}
              >
                {label}
